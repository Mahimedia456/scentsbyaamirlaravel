...
class InventoryController extends Controller
{
    ...
    public function adjust(Request $request)
    {
        ...
        DB::transaction(function()use($data){$product=Product::lockForUpdate()->findOrFail($data['product_id']);$variant=!empty($data['product_variant_id'])?ProductVariant::lockForUpdate()->where('product_id',$product->id)->findOrFail($data['product_variant_id']):null;$current=$variant?(int)$variant->stock:(int)$product->stock;$after=max(0,$current+(int)$data['quantity_change']);if($variant)$variant->update(['stock'=>$after]);else $product->update(['stock'=>$after,'stock_quantity'=>$after,'track_inventory'=>true,'is_in_stock'=>$after>0]);InventoryAdjustment::create($data+['quantity_after'=>$after,'user_id'=>auth()->id()]);});
