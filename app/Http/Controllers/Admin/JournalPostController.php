...
class JournalPostController extends Controller
{
    ...
    public function duplicate(JournalPost $journal_post){$copy=$journal_post->replicate();$copy->title=$journal_post->title.' Copy';$copy->slug=Str::slug($copy->title).'-'.Str::lower(Str::random(4));$copy->status='draft';$copy->published_at=null;$copy->save();return redirect()->route('admin.journal-posts.edit',$copy)->with('success','Journal post duplicated as draft.');}
