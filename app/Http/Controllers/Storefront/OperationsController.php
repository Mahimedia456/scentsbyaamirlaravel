...
class OperationsController extends Controller {
 ...
 public function trackStore(Request $r){
    $d=$r->validate([
        'order_number'=>'required|string|max:120',
        'identity'=>'required|string|max:190',
    ]);

    $reference=trim($d['order_number']);
    $identity=trim($d['identity']);

    $order=Order::with('items')
        ->where(function($q)use($reference){
            $q->where('order_number',$reference)
              ->orWhere('tracking_number',$reference);
        })
        ->where(function($q)use($identity){
            $q->whereRaw('LOWER(customer_email) = ?', [strtolower($identity)])
              ->orWhere('customer_phone',$identity);
        })
        ->first();

    return view('store.track-order',compact('order'))->with('searched',true);
}
