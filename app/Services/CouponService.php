...
class CouponService
{
    public function resolve(?string $code, Customer $customer, float $subtotal): array
    {
        ...
        if (!$coupon || !$coupon->isCurrentlyValid()) {
            throw ValidationException::withMessages(['coupon_code' => 'This promo code is invalid, inactive or expired.']);
        }
        if ($coupon->minimum_order !== null && $subtotal < (float) $coupon->minimum_order) {
            throw ValidationException::withMessages(['coupon_code' => 'This promo code requires a minimum order of PKR '.number_format((float) $coupon->minimum_order).'.']);
        }
        if ($coupon->usage_limit_per_customer) {
            ...
            if ($used >= (int) $coupon->usage_limit_per_customer) {
                throw ValidationException::withMessages(['coupon_code' => 'You have already used this promo code the maximum number of times.']);
            }
        }
